...
use App\Imports\BloodGroupImport;
...
class BloodGroupController extends Controller
{
    ...
    {
        Excel::import(new BloodGroupImport, $request->file('file')->store('temp'));
    ...
    public function sendBloodGroupMail(Request $request)
    {
        ...
        $mailData = BloodGroup::where('id', $patient_id)->firstOrFail();

        $findEmail = $mailData["patient_email"];

        $email = (Crypt::decryptString($findEmail));

        Mail::to($email)->send(new BloodGroupMail($mailData));
    ...
    {
        $result = BloodGroup::where('id', $id)->first();
    ...
    {
        $patients = BloodGroup::whereIn('id', $request->ids)->get();
        ...
        if($patients->count() > 0)
        {
            ...
            {
                Mail::to(Crypt::decryptString($mailData->patient_email))->send(new BloodGroupMail($mailData));
            }
        }
