...
class BloodGroupImport implements ToCollection, WithHeadingRow
{
    use Importable;
    /**
    * @param array $row
    ...
    public function collection(Collection $rows)
    {
        ...
        foreach ($rows as $row)
        {
            $check[] = BloodGroup::create([
                ...
                'document_number' => TestHelper::IDGenerator(new BloodGroup(), 'document_number', 6),
        ...
        foreach($check as $key => $result)
        {
            ...
            Mail::to(Crypt::decryptString($result->patient_email))->send(new BloodGroupMail($result));
