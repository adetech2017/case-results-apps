...
class HivImport implements ToCollection, WithHeadingRow
{
    use Loggable;
    use Importable;
    /**
    ...
    public function collection(Collection $rows)
    {
        ...
        foreach ($rows as $row)
        {
            $check[] = HivScreening::create([
                ...
                'document_number' => TestHelper::IDGenerator(new HivScreening(), 'document_number', 6),
        ...
        foreach($check as $key => $result)
        {
            ...
            Mail::to(Crypt::decryptString($result->patient_email))->send(new HivTest($result));
